...
// Savestate functions
extern uint32_t TomReadSavestate(unsigned char *ptrsst);
extern uint32_t TomWriteSavestate(unsigned char *ptrsst);
...
extern void TOMExecHalfline(uint16_t halfline, bool render);
...
extern int TOMIRQEnabled(int irq);
